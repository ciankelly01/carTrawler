...
import com.cartrawler.assessment.car.CorpSizeKey;
...
public class AssessmentRunner {
    ...
    static {
        ...
        CARS.add(new CarResult("Mercedes aCitan", "MARBESOL", "CMMD", 10.53d, CarResult.FuelPolicy.FULLEMPTY));
        CARS.add(new CarResult("BMW 4 Series Convertible", "MARBESOL", "RTAV", 189.85d, CarResult.FuelPolicy.FULLEMPTY));
        CARS.add(new CarResult("Renault Megane Convertible", "MARBESOL", "CTMR", 59.22d, CarResult.FuelPolicy.FULLEMPTY));
        CARS.add(new CarResult("Citroen Berlingo", "MARBESOL", "CMMV", 10.53d, CarResult.FuelPolicy.FULLEMPTY));
        CARS.add(new CarResult("Ford Focus Estate", "MARBESOL", "CWMR", 16.11d, CarResult.FuelPolicy.FULLEMPTY));
    ...
    static void main(String[] args) {
        List<CarResult> cars = new ArrayList<>(CARS);
        sortByCategory(cars);

        List<CarResult> filteredCars = new ArrayList<>(removeExpensiveFullFullByCategory(cars));

        Display display = new Display();
        display.render(filteredCars);
    }
    ...
    static int categoryOrder(CarResult car) {

        String sipp = car.getSippCode();

        if (sipp.startsWith("M")) return MINI;
        if (sipp.startsWith("E")) return ECONOMY;
        if (sipp.startsWith("C")) return COMPACT;
        return OTHER;                          // Other
    }

    private static boolean isCorporate(CarResult car) {
        return CORPORATE_SUPPLIERS.contains(car.getSupplierName());
    }

    private static void sortByCategory(List<CarResult> sortedCars) {

        sortedCars.sort(
                Comparator
                        //Corporate cars first
                        .comparing((CarResult car) -> !isCorporate(car))
                        //Then sipp order within each group
                        .thenComparing(AssessmentRunner::categoryOrder)
                        //Then price
                        .thenComparing(CarResult::getRentalCost)
        );
    }

    static List<CarResult> removeExpensiveFullFullByCategory(Collection<CarResult> cars) {

        // Group FULLFULL prices by (corporate + size)
        Map<CorpSizeKey, List<Double>> pricesByGroup =
                cars.stream()
                        .filter(car -> car.getFuelPolicy() == CarResult.FuelPolicy.FULLFULL)
                        .collect(Collectors.groupingBy(
                                car -> new CorpSizeKey(
                                        isCorporate(car),
                                        categoryOrder(car)   // size category
                                ),
                                Collectors.mapping(CarResult::getRentalCost, Collectors.toList())
                        ));

        // Calculate median FULLFULL price per group
        Map<CorpSizeKey, Double> medianByGroup = new HashMap<>();

        for (var entry : pricesByGroup.entrySet()) {
            List<Double> prices = entry.getValue();
            prices.sort(Double::compareTo);
            medianByGroup.put(entry.getKey(), median(prices));
        }

        // Remove FULLFULL cars priced above median in their (corporate + size) group
        return cars.stream()
                .filter(car -> {
                    ...
                    CorpSizeKey key = new CorpSizeKey(
                            isCorporate(car),
                            categoryOrder(car)
                    );

                    Double median = medianByGroup.get(key);
                    return median == null || car.getRentalCost() <= median;
                })
                .toList();
    }


    private static double median(List<Double> prices) {
